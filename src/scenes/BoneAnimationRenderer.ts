...
export class BoneRenderer {
    ...
    angleDiffDeg(a: number, b: number): number {
        let diff = b - a;
    ...
    init() {
        ...
        for (let index = 0; index < bones.frames.length - 1; index++) {
            ...
            if (b2) {
                ...
                b1Bones.forEach((bone, bIndex) => {
                    ...
                    // const nextAngle = Phaser.Math.Angle.WrapDegrees(nextFrameBone.angleDegree);
                    // const curAngle = Phaser.Math.Angle.WrapDegrees(bone.angleDegree);
                    const nextAngle = nextFrameBone.angleDegree;
                    const curAngle = bone.angleDegree;

                    const angleSpeed = this.angleDiffDeg(curAngle, nextAngle) / diffFrameTime;
        ...
        Object.values(firstFrameBones).forEach((bone) => {
            ...
            s.setScale(0.2);
            s.setRotation(Phaser.Math.Angle.WrapDegrees(bone.angle));
            s.setOrigin(0.5, 1);
